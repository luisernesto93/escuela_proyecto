...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="font-weight-bold text-primary"><i class="fas fa-user-edit"></i> EDITAR PLAN DE ESTUDIOS</h6>
        </div>
        <form method="POST" action="{{route('plan_estudios.update', ['plan_estudio' => $plan_estudio->id])}}">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="form-group row">
                    ...
                    <div class="col-sm-3 mb-3 mt-3 mb-sm-0">
                        {{--<label for="resolucion_id"> --}}<strong>Resoluci&oacute;n: </strong><span style="color:red;">*</span></label>
                        ...
                        required>
                            ...
                            @foreach ($resoluciones as $resolucion)
                                <option value="{{$resolucion->id}}" {{ old('resolucion_id', $plan_estudio->resolucion_id) == $resolucion->id ? 'selected' : ''}} >{{$resolucion->numero_resolucion}}</option>
                            @endforeach
                        </select>
                        @error('resolucion_id')
            ...
            <div class="card-footer">
                ...
                <a class="btn btn-outline-danger float-right mr-3 mb-3" href="{{ route('plan_estudios.index') }}"><i class="fas fa-window-close"></i> CANCELAR</a>
            </div>
        </form>
    </div>
</div>
