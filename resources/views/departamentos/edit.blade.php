...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="font-weight-bold text-primary"><i class="fas fa-user-plus"></i> ACTUALIZAR DEPARTAMENTO</h6>
        </div>
            ...
            <div class="card-body">
                <div class="form-group row">
                    ...
                    <div class="col-sm-3 mb-3 mt-3">
                        ...
                        <select class="form-control form-control-sm @error('pais_id') is-invalid @enderror" name="pais_id" required>
                            <option value="" selected>Seleccione Pa&iacute;s</option>
                            @foreach ($paises as $pais)
                                <option value="{{$pais->id}}" {{ old('pais_id', $departamento->pais_id) == $pais->id ? 'selected' : ''}}> {{$pais->nombre}} </option>
                            @endforeach
                        </select>
                        @error('pais_id')
                            <span class="text-danger">{{$message}}</span>
