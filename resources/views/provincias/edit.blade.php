...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="font-weight-bold text-primary"><i class="fas fa-user-plus"></i> ACTUALIZAR PROVINCIA</h6>
        </div>
        <form method="POST" action ="{{route('provincias.update', ['provincia' => $provincia->id])}}">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="form-group row">
                    ...
                    <div class="col-sm-3 mb-3 mt-3">
                        ...
                        <select class="form-control form-control-sm @error('departamento_id') is-invalid @enderror" name="departamento_id"
                        required>
                            <option value="" selected>Seleccione Departamento</option>
                            @foreach ($departamentos as $departamento)
                                <option value="{{$departamento->id}}" {{ old('departamento_id', $provincia->departamento_id) == $departamento->id ? 'selected' : ''}}> {{$departamento->nombre}} </option>
                            @endforeach
                        </select>
            ...
            <div class="card-footer">
                ...
                <a class="btn btn-outline-primary float-right mr-3 mb-3" href="{{ route('provincias.index') }}"><i class="fas fa-arrow-left fa-sm text-primary-100"></i> CANCELAR</a>
            </div>
        </form>
    </div>
